...
- **سلسلة ذاتية التغذية (بديل كامل لجدولة GitHub)**: جدولة cron لم تعمل إطلاقاً على مستودع
  جديد (إشعار "تفعيل الجدولة" لا يظهر للجميع، ولا يوجد API له). الحل المطبق:
  - سر `DISPATCH_PAT` (Pat كلاسيكي بصلاحية `workflow` فقط) يُستخدم في خطوة
    `Self-dispatch (keep chain alive)` في نهاية كل تشغيل: يفتح `actions/runs`،
    إن لم توجد عملية أخرى (باستثناء نفسها) يطلق `gh workflow run goldgap.yml`.
  - النتيجة: تشغيلة كل ~5 دقائق إلى الأبد مجاناً، مؤكدة عملياً بثلاث دورات متتالية
    (33334586193 ← 33334823499 ← 33335035793) بدون أي تدخل، ومع
    `concurrency` (goldgap) فلا يحدث تداخل/تكرار سياسة أبداً حتى لو عمل cron أو
    dispatch خارجي إضافي.
  - المهام: `timeout-minutes: 8`، مدة الحلقة `CBOT_DURATION_MIN=4.5`.
...
### إصلاحات حرجة (أغسطس 2026 — مُثبتة في السحابة) 🔧

- **مقياس السعر/الربح (مهم جداً):** `pos.price` في كائن position قادم **كـ float بالدولار الحقيقي** (مثال `4428.76`) بلا أي مقياس. `mid` (من البث) أيضًا بالدولار. قيمة الربح الخام `(mid - entry) × volume` تخرج **مضروبة في `10^md`** (moneyDigits=2 → القسمة على 100) بنفس مقياس `commission`/`swap`. لولا القسمة على `10^md` ظهر ربح وهمي ضخم (أظهر `pnl_net_usd=439000` بدل ~$5). الحل في `dynamic_pnl_usd`: `gross = (mid - entry) * volume / (10 ** md)` ثم `pnl_net = gross - fees`. التصحيح مثبت ويُعطي أرقاماً واقعية (رصيد 1008.79$).
- **الفتح المزدوج (طلب المالك صراحة — لا صفقة ثانية طالما الأولى مفتوحة):** السبب كان مسارين:
  1. `FORCE_TEST_OPEN` يفتح دائماً بلا تحقق → أُصلح: يتخطى (`FORCE-TEST SKIP`) إذا وُجدت أي صفقة مفتوحة.
  2. cache `open_positions(max_age=6)` يُرجع قائمة فارغة خاطئة بعد فتح/إغلاق عبر عمليات أخرى → أُصلح: `open_market` تضيف الصفقة للكاش و`close_position` تزيلها، وأُضيف **حارس reconcile طازج (`max_age=0`) قبل أي فتح** في `run_trade_cycle` — إن وُجدت صفقة يعطي `action=hold:already_open` ولا يفتح أبداً.
- **أسرار `STRAT_*` لم تكن تصل للسحابة أصلاً:** الـ workflow كان يمرر فقط `CBOT_*` envs، فكل `_env_bool("STRAT_FORCE_TEST_OPEN"...)=False` في السحابة واختبار المقاييس لم يعمل أبداً رغم وجود السر. الحل: أُضيفت `STRAT_*` كـ env في `goldgap.yml` (STRAT_COOLDOWN_MIN, STRAT_FORCE_TEST_OPEN, STRAT_FEES_PER_LOT, STRAT_PROFIT_FLOOR, STRAT_PROFIT_FLOOR_LOT).
- **`err=TimeoutError`:** رُفعت مهلات `get_trader`/`open_positions` من 15→30 ثانية، مع تخبئة `open_positions` بـ`max_age` لتقليل الطلبات المتكدسة. قد يظهر `TimeoutError(30)` أحياناً لكنه لا يمنع أو يفتح صفقات (المعاملات محمية بالحارس).
- **`TRADING_BAD_STOPS` (لم يُحسم بعد):** البروكر يرفض وضع SL/TP سواءً مرفقاً بالفتح أو عبر `ProtoOAAmendPositionSLTPReq`؛ الفتح ينجح والصفقة تبقى بلا حماية. اختبار المقاييس الآن يصل للسحابة (بعد إصلاح الـ STRAT_* secrets أعلاه) وسيُظهر أي مقياس يقبله (SPOT_SCALE=100000 أم 100 أم مسافات). راجع مصادر السوق قبل تعديله.
